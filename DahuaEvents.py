...
class DahuaEvents(hass.Hass):
                    ...
                    if response.status_code == 200:
                        ...
                        for line in response.iter_lines(chunk_size=512):
                            if line:
                                ...
                                # No necesitamos el nombre de la regla, por lo que simplemente procesamos el evento
                                self.process_event(decoded_line, camera)
    ...
    def process_event(self, decoded_line, camera):
        """
        Procesa el evento recibido, activando el sensor correspondiente.
        """
        if "Code=CrossLineDetection" in decoded_line:
            self.handle_detection_event("CrossLineDetection", camera)

        elif "Code=CrossRegionDetection" in decoded_line:
            self.handle_detection_event("CrossRegionDetection", camera)

    def handle_detection_event(self, event_code, camera):
        """
        Maneja la lógica de activación y desactivación de los sensores para un evento dado.
        """
        current_time = datetime.now().strftime("%H:%M:%S")
        self.log(f"Evento IVS detectado - {event_code} - Hora: {current_time} - Cámara: {camera['name']}")
